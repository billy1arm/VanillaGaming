...
void Player::UpdateCombatSkills(Unit* pVictim, WeaponAttackType attType, bool defence)
{
    ...
    int32 skilldif = 5 + 5 * plevel - (defence ? GetBaseDefenseSkillValue() : GetBaseWeaponSkillValue(attType));
            ...
            else
            {
                ...
                else
                {
                    if (Group* group = creature->GetGroupLootRecipient())
                    {
                        if (group == GetGroup())
                        {
                            if (group->GetLootMethod() == FREE_FOR_ALL || group->GetLootMethod() == ROUND_ROBIN)
                                permission = ALL_PERMISSION;
                            else if (group->GetLooterGuid() == GetObjectGuid())
                            {
                                if (group->GetLootMethod() == MASTER_LOOT)
...
bool Player::isAllowedToLoot(Creature* creature)
{
    ...
    if (Player* recipient = creature->GetLootRecipient())
    {
        ...
        {
            if (Group* thisGroup = GetGroup())
            {
                /* Check groups are the same */
                if (otherGroup != thisGroup)
                    { return false; } // Cheater, deny loot
            }
            else
                { return false; } // We're not in a group, probably cheater
            ...
            switch (otherGroup->GetLootMethod())
            {
                ...
                case NEED_BEFORE_GREED:
                {
                    ...
                    * If this is true, allow everyone else in the group to loot the corpse */
                    ...
                    Group::MemberSlotList slots = otherGroup->GetMemberSlots();
