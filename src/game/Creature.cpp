...
bool Creature::InitEntry(uint32 Entry, Team team, CreatureData const* data /*=NULL*/, GameEventCreatureData const* eventData /*=NULL*/)
{
    ...
    if (cinfo->InhabitType & INHABIT_WATER &&                                                                 // 检测生物行动特征是否包含游泳
            !(cinfo->ExtraFlags & CREATURE_EXTRA_FLAG_WALK_IN_WATER) &&                                       // 检测生物是否强制步行(螃蟹,巨人...)
            data &&                                                                                           // check if there is data to get creature spawn pos                                                                // check if there is data to get creature spawn pos
            GetMap()->GetTerrain()->IsSwimmable(data->posX, data->posY, data->posZ, minfo->bounding_radius))  // 检测生物是否在水中且有足够的空间游泳
...
void Creature::DoFleeToGetAssistance()
{
    ...
    if (radius > 0)
    {
        ...
        UpdateSpeed(MOVE_RUN, false, 0.8f);
