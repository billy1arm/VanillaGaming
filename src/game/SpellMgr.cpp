...
bool SpellMgr::IsNoStackSpellDueToSpell(uint32 spellId_1, uint32 spellId_2) const
{
            ...
            spellInfo_1->SpellIconID != 0 && spellInfo_2->SpellIconID != 0)
    {
        ...
        for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
        {
            if (spellInfo_1->EffectApplyAuraName[i] == SPELL_AURA_ADD_FLAT_MODIFIER || spellInfo_1->EffectApplyAuraName[i] == SPELL_AURA_ADD_PCT_MODIFIER || spellInfo_1->EffectApplyAuraName[i] == SPELL_AURA_MOD_INCREASE_SPEED || spellInfo_2->EffectApplyAuraName[i] == SPELL_AURA_ADD_FLAT_MODIFIER || spellInfo_2->EffectApplyAuraName[i] == SPELL_AURA_ADD_PCT_MODIFIER || spellInfo_2->EffectApplyAuraName[i] == SPELL_AURA_MOD_INCREASE_SPEED)
                { isModifier = true; }
        }
