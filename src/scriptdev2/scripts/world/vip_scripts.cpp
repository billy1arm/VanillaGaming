        ...
        case GOSSIP_ACTION_INFO_DEF + 7:
            if (pPlayer->isAlive())
            {
                ...
                else
                {
                    ...
                    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, vip_scripts_To_UTF8("卷轴:火眼莫德雷斯-消耗450积分"), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 73);     // 卷轴:火眼莫德雷斯
                    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, vip_scripts_To_UTF8("返回主菜单"), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);                         // 返回主菜单
                    pPlayer->TalkedToCreature(pCreature->GetEntry(), pCreature->GetObjectGuid());
                    pPlayer->SendPreparedGossip(pCreature);
                }
        ...
        case GOSSIP_ACTION_INFO_DEF + 70:
            if (pPlayer->isAlive())
            {
                ...
                else
                {
                    if (pPlayer->GetIntegral() >= 450)
                    {
                        if (pPlayer->GetItemCount(30015, true) < 1)
                        {
                            ItemPosCountVec dest;
                            InventoryResult msg = pPlayer->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, 30015, 1, (uint32)0);
                            if (msg == EQUIP_ERR_OK)
                            {
                                pPlayer->SaveToDB();
                                pPlayer->ModifyIntegral(-450);
                                Item* item = pPlayer->StoreNewItem(dest, 30015, true, Item::GenerateItemRandomPropertyId(30015));
                                pPlayer->SendNewItem(item, 1, false, true);
                                pPlayer->SaveToDB();
                                ChatHandler(pPlayer).PSendSysMessage(LANG_USE_INTEGRAL, 450);
                            }
        ...
        case GOSSIP_ACTION_INFO_DEF + 71:
            if (pPlayer->isAlive())
            {
                ...
                else
                {
                    if (pPlayer->GetIntegral() >= 450)
                    {
                        if (pPlayer->GetItemCount(30016, true) < 1)
                        {
                            ItemPosCountVec dest;
                            InventoryResult msg = pPlayer->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, 30016, 1, (uint32)0);
                            if (msg == EQUIP_ERR_OK)
                            {
                                pPlayer->SaveToDB();
                                pPlayer->ModifyIntegral(-450);
                                Item* item = pPlayer->StoreNewItem(dest, 30016, true, Item::GenerateItemRandomPropertyId(30016));
                                pPlayer->SendNewItem(item, 1, false, true);
                                pPlayer->SaveToDB();
                                ChatHandler(pPlayer).PSendSysMessage(LANG_USE_INTEGRAL, 450);
                            }
        ...
        case GOSSIP_ACTION_INFO_DEF + 73:
            if (pPlayer->isAlive())
            {
                ...
                else
                {
                    if (pPlayer->GetIntegral() >= 450)
                    {
                        if (pPlayer->GetItemCount(30020, true) < 1)
                        {
                            ItemPosCountVec dest;
                            InventoryResult msg = pPlayer->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, 30020, 1, (uint32)0);
                            if (msg == EQUIP_ERR_OK)
                            {
                                pPlayer->SaveToDB();
                                pPlayer->ModifyIntegral(-450);
                                Item* item = pPlayer->StoreNewItem(dest, 30020, true, Item::GenerateItemRandomPropertyId(30020));
                                pPlayer->SendNewItem(item, 1, false, true);
                                pPlayer->SaveToDB();
                                ChatHandler(pPlayer).PSendSysMessage(LANG_USE_INTEGRAL, 450);
                            }
